...
async function run() {
  ...
  if (!owner || !repo || !me || !branch) {
    owner, repo, me;
    console.warn(
      "This command requires 4 arguments, The GitHub owner, repo, user, and branch respectively."
    );
    console.warn(
      `For instance: node open-pr.js 'firefox-devtools' 'profiler' 'gregtatum' 'my-branch-name'`
    );
    process.exit(1);
  }
  ...
  const pr = response.data.find(
    (pr) => pr.head.ref === branch && pr.user.login === me
  );
  if (!pr) {
